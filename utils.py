...
def read_genmol(sequence, inv_smi_vocab):
    """
    Converts a sequence of token indices into a SMILES string using the inverse SMILES vocabulary.

    Args:
        sequence (list): A list of token indices representing a generated molecule.
        inv_smi_vocab (dict): A dictionary mapping token indices to SMILES characters.

    Returns:
        str: The generated SMILES string.
    """
    # Decode the token indices into SMILES characters
    smiles = ''.join([inv_smi_vocab[idx] for idx in sequence if idx in inv_smi_vocab])

    # Remove unwanted tokens like [START], [END], and [PAD]
    smiles = smiles.replace("[START]", "").replace("[END]", "").replace("[PAD]", "")
...
# print(f"SMILES list: {smi_list[:10]}")  # Print the first 10 SMILES strings
